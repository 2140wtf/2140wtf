// Verify the Buy It Now dialog with an authenticated session.
// Injects a plaintext login blob into localStorage under the app's
// `nostr:login` key using an ephemeral throwaway nsec — exercises the
// logged-in path (fixed price, locked input, Confirm button) without
// touching any real user session. Browser context is discarded afterwards.
...
// Confirm logged-in state via toolbar
const designAuction = page.getByRole('button', { name: /design auction/i }).first();
console.log('Logged in (Design auction visible):', await designAuction.isVisible().catch(() => false));
