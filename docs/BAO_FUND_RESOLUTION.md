...
| **L0.5 Advisory** | AI scorer (kind 38037, declared `verifier_pubkeys`) tool-verifies delivery evidence and publishes an advisory verdict that can <i>recommend</i> an objection and informs donor L2 votes — never binding | low (judge fee, capped) | no |
...
- **AI scores are advisory (L0.5).** kind-38037 scores are valid only from the
  campaign's declared `verifier_pubkeys` and never gate payouts — they surface
  early-objection recommendations and donor decision-support (§2). Only L1–L3
  artifacts resolve payouts.
