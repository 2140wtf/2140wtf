...
export function encodeEnvelopeEvent(authorSecretKey, envelope, ctx, clock = systemClock, rng = defaultRng, policy = DEFAULT_PRIVACY_POLICY) {
    // Fail-fast binding (mirror of giftWrapEnvelope): the outer signer MUST
    // own envelope.author — decodeEphemeralEvent enforces inner==outer, so a
    // mismatched key minted an event every reader rejects with no local error.
    if (getPublicKey(authorSecretKey) !== envelope.author) {
        throw new Error('encodeEnvelopeEvent: signer does not match envelope.author — readers would reject this event');
    }
    return signEvent({
        kind: EPHEMERAL_MESSAGE,
        created_at: privacyTimestamp(clock, rng, policy),
