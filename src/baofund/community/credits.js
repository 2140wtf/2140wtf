...
export function foldCredits(messages) {
    ...
    for (const { author, op } of pending) {
        ...
        if (op.op === 'fulfill') {
            ...
            const payout = state.request.from?.toLowerCase();
            if (payout && op.to.toLowerCase() !== payout)
                continue;
            // The funder's identity is the DURABLE key it declared (`from`), not
            // the per-room stream key that signed the envelope. Receipts name
            // durable identities, and corroboratedFunders compares against these
            // keys — keying by envelope author made every real fleet fulfill
            // uncorroboratable (and let the requester self-fund across a key
            // rotation by declaring its own payout key from a second room key).
            const funderIdentity = (op.from ?? author).toLowerCase();
            if (payout && funderIdentity === payout)
                continue; // same principal, self-funding
            state.funderClaims.set(funderIdentity, op);
        }
        else {
            if (author.toLowerCase() !== state.requester.toLowerCase())
                continue; // receipts are self-reports
            // Receipts dedupe per request (doc contract): N receipts for one request
            // fold to 1. Keep the latest in scroll order ("latest shown") — duplicate
            // or replay-posted receipts must not multiply folded state.
            state.receipts = [op];
        }
    }
    const fundedIds = [];
