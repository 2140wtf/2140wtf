...
import { buildCreditRequest, buildCreditFulfill, buildCreditReceipt, newCreditId, sealTo, } from './credits.js';
...
export class AgentFleet {
    ...
    async join(link, opts = {}) {
        // Mention scoping is the fleet's contract: mentions()/unreadMentions()
        // watch THIS fleet's durable identity. Claiming a different agentPub on
        // the join would admit the agent under an identity the fleet never
        // listens for (silent mention drop) — reject it up front, before any
        // network I/O.
        if (opts.agentPub && opts.agentPub.toLowerCase() !== this.agentPub) {
            throw new Error(`fleet join: claimed agentPub ${opts.agentPub.toLowerCase()} does not match this fleet's agentPub ${this.agentPub} — mentions() watches the fleet identity`);
        }
    ...
    /** Post a compute/work credit request. Returns the request id. The fleet's
     *  durable pubkey (agentPub) rides as the funder target (`from`), so
     *  foldCredits binds fulfills to the key the fleet can actually spend. */
    async requestCredits(roomId, amountSats, purpose) {
        const id = newCreditId();
        const envelope = await this.require(roomId).post(buildCreditRequest({ id, amountSats, purpose, from: this.agentPub }));
        return { id, envelope };
    }
    /** Fund a credit request. `from` (or the pubkey derived from
     *  `opts.sealWithSecretKey`) is the funder's DURABLE pubkey — REQUIRED when
     *  a sealedToken is present, so the requester can unseal (the room envelope
     *  author is a per-room stream key). Built through buildCreditFulfill:
     *  a sealed token without a funder binding is refused, never posted as an
     *  unusable payload. */
    async fulfillCredits(roomId, args, opts = {}) {
        const derivedFrom = opts.sealWithSecretKey ? getPublicKey(opts.sealWithSecretKey) : undefined;
        if (derivedFrom && args.from && args.from.toLowerCase() !== derivedFrom.toLowerCase()) {
            throw new Error('fleet fulfill: from does not match the supplied sealWithSecretKey');
        }
        const from = derivedFrom ?? args.from;
        const fulfill = buildCreditFulfill({
        ...
        return this.require(roomId).post(fulfill);
    }
