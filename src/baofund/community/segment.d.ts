...
export interface SegmentContext {
    ...
    epoch?: number;
    /** P2 grace (§3.1): the previous epoch's content key/epoch, when the
     *  reader still holds them. A roll race can land an in-flight
     *  previous-epoch envelope in a CURRENT-scope segment (the blind scribe
     *  cannot epoch-filter); the live subscription accepts those via
     *  P2_EPOCH_WINDOW — the scroll path must open them too, or read()
     *  silently drops messages the live path delivered. */
