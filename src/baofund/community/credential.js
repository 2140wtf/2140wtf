 ...
 * pattern) — TTL alone leaves the cache unbounded under a credential flood.
 * Under sustained pressure the soonest-expiring (usually oldest) unexpired
 * nullifiers are evicted first, so a credential whose nullifier was evicted
 * can be replayed before its natural TTL would have closed the window. The
 * deliberate trade-off: exhaustive replay protection while under the cap
 * (every realistic welcomer window), bounded memory under a flood. Raise the
 * cap or persist nullifiers durably when the deployment needs more.
 */
export class NullifierCache {
    ...
    /** Over the cap: expired first, then soonest-expiring survivors. */
    evictOverCap() {
        if (this.seen.size < this.limit)
            return;
        this.sweep();
        if (this.seen.size >= this.limit) {
            const soonest = [...this.seen.entries()].sort((a, b) => a[1] - b[1]);
            for (let i = 0; i < soonest.length && this.seen.size >= this.limit; i++) {
                this.seen.delete(soonest[i][0]);
            }
        }
    }
    get size() {
