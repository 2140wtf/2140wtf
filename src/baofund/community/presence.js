...
/** Presence names are rendered in rosters and used as mention handles —
 *  Unicode control/format characters (NUL, C0/C1, bidi overrides and
 *  isolates, line/paragraph separators) let a member present a visually
 *  reordered or hidden look-alike that never collides with the name it
 *  impersonates (the collision pass compares raw strings). */
...
export function buildPresence(name, opts = {}) {
    ...
    if (UNSAFE_NAME_CHARS.test(trimmed))
        throw new Error('name must not contain control or bidi characters');
...
function nextMention(lower, entries, pos) {
    ...
    for (const e of entries) {
        ...
        // A glued occurrence must be SKIPPED, never accepted (the old `break`
        // accepted '@alice' inside '@alicex' and then stopped searching). Both
        // boundaries are checked: the char BEFORE must not be a word char or
        // another '@' (so 'bob@alice.com' is an email, not a mention).
        let i = lower.indexOf(needle, pos);
        while (i !== -1) {
            const before = i > 0 ? lower[i - 1] : '';
            const after = lower[i + needle.length];
            const leftOk = i === 0 || !/[a-z0-9_@]/.test(before);
            const rightOk = after === undefined || !/[a-z0-9_]/.test(after);
            if (leftOk && rightOk) {
                if (!best || i < best.at)
                    best = { entry: e, at: i, len: needle.length };
                break; // first valid occurrence for this entry is enough
            }
