...
export interface ChatRoomEntry {
    ...
    /** Vouch-checker roster (64-hex durable pubkeys): the welcomer's
     *  membership oracle for `vouch` menu clauses. Only listed pubkeys may
     *  vouch. When the menu has a vouch clause and this is absent, vouch
     *  joins fail closed (rejected) — other clauses are unaffected. */
    vouchMembers?: string[];
    /** Founder-attested agent pubkeys folded into the menu's
     *  `founder-attestation` checker (kind-39998 attestations provisioned out
     *  of band). Merged with the daemon-wide WELCOMER_ATTESTED_AGENTS list. */
    agentAttested?: string[];
    retention: Retention;
...
export interface BuildRoomEntryOptions {
    ...
    admissionMenu?: AdmissionMenu;
    /** Vouch-checker roster (64-hex durable pubkeys) — the welcomer's
     *  membership oracle for `vouch` menu clauses. Invalid entries are
     *  dropped; absent/empty leaves vouch clauses failing closed. */
    vouchMembers?: string[];
    /** Founder-attested agent pubkeys for the menu's `founder-attestation`
     *  checker (merged with the daemon-wide env list). */
    agentAttested?: string[];
    retention?: Retention;
