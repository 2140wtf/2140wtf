...
export function evaluateInvite(config, ctx) {
    ...
    // OWN properties only: the config is a plain object, so a lid like
    // "constructor"/"toString"/"__proto__" would otherwise resolve to
    // Object.prototype members (truthy), sailing past the expiry/uses checks
    // and admitting an expired or exhausted invite.
    const spec = Object.prototype.hasOwnProperty.call(config, ctx.lid) ? config[ctx.lid] : undefined;
    if (!spec)
...
export function parseInviteConfig(raw) {
    ...
    const push = (lid, spec) => {
        ...
        const history = s.history === 'fresh' || s.history === 'full' ? s.history : undefined;
        out[lid] = { lid, maxUses: Math.floor(s.maxUses), expiry, ...(history !== undefined ? { history } : {}) };
    };
