...
export function buildRoomEntry(name, topic = '', opts = {}) {
    ...
    return {
        ...
        ...(opts.vouchMembers?.length ? { vouchMembers: opts.vouchMembers.map((s) => s.toLowerCase()).filter((s) => /^[0-9a-f]{64}$/.test(s)) } : {}),
        ...(opts.agentAttested?.length ? { agentAttested: opts.agentAttested.map((s) => s.toLowerCase()).filter((s) => /^[0-9a-f]{64}$/.test(s)) } : {}),
