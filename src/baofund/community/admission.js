 ...
 *
 *  MEMORY BOUND (round-6): a hard entry cap with per-key eviction (TtlKeySet
 *  pattern: oldest inserted first). The rate bound alone is not a memory
 *  bound — a sybil voucher flood grows the map without limit. Under sustained
 *  pressure the oldest voucher's count is evicted, so that member gets a
 *  fresh vouch budget (over-admission risk bounded by the cap size). The
 *  trade-off is deliberate: bounded memory beats an unbounded map, and a
 *  deployment needing strict per-member caps must size the budget to its
 *  membership (or persist counts). */
export class VouchBudget {
    constructor(maxPerMember = 3, 
    /** Hard cap on tracked vouchers. */
    limit = 4096) {
    ...
    consume(voucher) {
        ...
        if (n >= this.maxPerMember)
            return false;
        if (!this.counts.has(voucher) && this.counts.size >= this.limit) {
            // Evict the oldest tracked voucher (Map preserves insertion order);
            // existing keys only ever increment up to maxPerMember, so order is
            // first-consume order.
    ...
    /** Tracked voucher keys (test/ops introspection). */
    get size() {
...
async function commitClause(clause, proofs, ctx, providers) {
    ...
    // Collect the clause's DISTINCT side effects first (dedup by nullifier /
    // voucher): duplicate checker refs must not double-consume.
    ...
    for (const ref of clause.and) {
        if (typeof ref === 'object' && ref.checker === 'trade-credential') {
            ...
            nullifiers.add(proofs.credential.credential.nullifier);
        }
        if (typeof ref === 'object' && ref.checker === 'vouch') {
            ...
            vouchers.add(parsed.voucher);
        }
    }
    // PRE-FLIGHT every consume before mutating ANY state: a lost race on a
    // later checker must not leave an earlier checker's nullifier burned on an
    // overall-rejected join (review finding 4 — the two-phase discipline
    // applies to commit-time races too, not just to read-only evaluation).
    const cache = providers.nullifierCache;
    for (const n of nullifiers) {
        if (!cache || cache.has(n)) {
            return { ok: false, reason: 'trade-credential commit: nullifier spent by a concurrent join' };
        }
    }
    const budget = providers.vouchBudget;
    for (const v of vouchers) {
        if (!budget || !budget.peek(v)) {
            return { ok: false, reason: 'vouch commit: budget spent by a concurrent join' };
        }
    }
    // APPLY — no await between pre-flight and apply, so the checked state still
    // holds (JS is single-threaded; another join cannot interleave here).
    if (nullifiers.size > 0 && proofs.credential && cache) {
