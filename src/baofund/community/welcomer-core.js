 ...
 */
export class InviteLedgerWriteError extends Error {
    constructor(lid, message, 
    ...
    reason = 'write') {
        ...
        this.lid = lid;
        this.reason = reason;
        this.name = 'InviteLedgerWriteError';
    }
}
/**
...
export class InviteUseLedger {
    ...
    commit(lid) {
        ...
        if (n <= 0)
            return;
        this.pending.set(lid, n - 1);
        this.committed.set(lid, (this.committed.get(lid) ?? 0) + 1);
    }
...
// NOTE: there is deliberately NO global ratchet rate-limit primitive. The
// spec defines join-forward at epoch granularity and no minimum epoch
// interval; the actual churn invariant is: a ratchet requires a join that
// passed admission AND a room/lid provisioned fresh AND a burner that has
// not already ratcheted within the wrap TTL (JOIN-07). The stateful half
// (atomic invite consume, provisioning gate, per-burner TTL dedup,
// per-room ratchet serialization) lives in welcomer-join.ts.
