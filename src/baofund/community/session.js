 ...
 */
import { systemClock, defaultRng, getPublicKey, bytesToHex, hexToBytes, generateSecretKey, deriveEpochKeys, ratchetEpoch, deriveScrollWrapperKey, scrollScope, privacyPolicyFor, encryptToRoomKey, decryptWithRoomKey, constantTimeEqual, } from './crypto.js';
...
export class RoomSession {
     ...
     *  the sender on shielded rooms. `authorSecretKey` must be the key that
     *  SIGNED the event (default: the current stream key; a republished
     *  pre-re-key event passes the retired key that owns it). */
    async publishEventInternal(event, authorSecretKey = this.joined.authorSecretKey) {
        if (this.joined.shieldPub) {
            const wrap = giftWrapEnvelope(event, this.joined.shieldPub, authorSecretKey, {
                rng: this.rng,
                clock: this.clock,
    ...
    async republish(envelope) {
        ...
        // The outer signer MUST match envelope.author (decodeEphemeralEvent
        // enforces inner vs outer equality). After a re-key the stream key
        // rotates and the ORIGINAL key moves to retiredAuthorSecretKeys — sign
        // the retry with the key that OWNS this msg_id, never blindly with the
        // current stream key (which would make every reader drop the retry).
        const signer = this.authorKeyFor(envelope.author);
        if (!signer) {
            throw new Error(`cannot republish envelope authored by ${envelope.author.slice(0, 12)}… — key not held by this session`);
        }
        await this.publishEventInternal(encodeEnvelopeEvent(signer, restamped, this.ctx(), this.clock, this.rng, this.joined.privacy), signer);
    }
    /** Secret key owning `authorPub` — current stream key or a retained one. */
    authorKeyFor(authorPub) {
        const want = authorPub.toLowerCase();
        if (getPublicKey(this.joined.authorSecretKey).toLowerCase() === want)
            return this.joined.authorSecretKey;
        for (const key of this.joined.retiredAuthorSecretKeys) {
            if (getPublicKey(key).toLowerCase() === want)
                return key;
        }
        return undefined;
    }
    subscribeLive(onEnvelope) {
        ...
        // Ref-count PER SUBSCRIPTION, never per callback identity: a consumer may
        // legitimately subscribe the same (often bound) function twice, and each
        // unsubscribe releases exactly ONE subscription. Storing `onEnvelope`
        // directly in the Set collapsed those into one entry — the first
        // unsubscribe closed the shared relay REQ while the second subscription
        // was still outstanding (silent deafness).
        const wrapped = (env, event) => onEnvelope(env, event);
        this.liveHandlers.add(wrapped);
        return () => {
            this.liveHandlers?.delete(wrapped);
    ...
    applyRekey(newChainKey, newEpoch) {
        // Validate the WHOLE input before touching any state (atomicity): a
        // malformed re-key wrap used to rotate the stream key first and then
        // throw from deriveEpochKeys, leaving a half-applied re-key behind.
        // And a non-integer epoch (NaN) slipped past `newEpoch <= epoch` — the
        // comparison is false for NaN — corrupting epoch/chain state into a
        // session that could no longer be serialized/restored.
        if (!Number.isSafeInteger(newEpoch) || newEpoch < 0) {
            throw new Error('re-key: epoch must be a non-negative safe integer');
        }
        if (!(newChainKey instanceof Uint8Array) || newChainKey.length !== 32) {
            throw new Error('re-key: chain key must be 32 bytes');
        }
    ...
    async readCurrentEpoch(sinceSec) {
        const segCtx = {
            ...
            routingId: this.joined.routingId,
            epoch: this.joined.epoch,
            // P2 grace (§3.1): pass the retained previous-epoch key so a roll
            // race's in-flight envelope packed into this scope's segment still
            // opens during read() (the live path already accepts it).
            ...(this.joined.previousEncKey && this.joined.previousEpoch !== undefined
                ? { previousEncKey: this.joined.previousEncKey, previousEpoch: this.joined.previousEpoch }
                : {}),
        };
        const segmentEvents = [];
