...
export interface JoinOptions {
    ...
    agentAuth?: string[];
    /** Durable member-claim admission (F7 welcomer half): schnorr proof that
     *  the joiner controls this durable room-scoped member key. Rides INSIDE
     *  the encrypted join request (never relay-visible); the welcomer gates
     *  on it when the room provisions a member policy ('all'/'selected').
     *  Agent parity: identical for agents and humans, no personhood. */
