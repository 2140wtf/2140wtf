...
export function mergeScrolls(events, opts) {
    ...
    for (const [scribe, stream] of streams) {
        ...
        for (const item of stream) {
            ...
            // Cross-scribe tie-break is msg_id (spec §3.3); the AUTHOR is appended
            // as a final fallback because msg_id is only unique PER AUTHOR — two
            // byzantine authors can share one msg_id, and without the fallback the
            // two entries tie, making the emitted order depend on input order
            // (non-deterministic merge across clients).
            const orderKey = `${String(item.seg).padStart(12, '0')}:${String(item.pos).padStart(8, '0')}:${item.envelope.msg_id}:${item.envelope.author}`;
