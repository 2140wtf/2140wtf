...
export function privacyPolicyFor(policy) {
    ...
    if (typeof policy === 'number') {
        // Non-finite numbers are config errors: NaN used to flow through
        // Math.max/Math.floor into `windowSec: NaN`, and privacyTimestamp then
        // minted `created_at: NaN` (serialized as null) — a signed event no
        // relay can scroll. Fail loudly instead of poisoning the policy.
        if (!Number.isFinite(policy))
            throw new Error(`bad privacy policy windowSec: ${String(policy)}`);
