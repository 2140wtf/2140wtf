...
export interface InviteSpec {
    ...
    expiry: number;
    /** Per-link history provisioning (§8 JOIN-02): 'fresh' lets joins through
     *  this lid request a clean-slate ratchet. Absent/'full' → a join that
     *  claims history:'fresh' degrades to the current epoch (no ratchet). */
    history?: 'full' | 'fresh';
}
