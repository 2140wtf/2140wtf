...
export function looksLikeNip44V2(s) {
    ...
    if (!/^[A-Za-z0-9+/]+={0,2}$/.test(s))
        return false;
    // decode first byte → must be 0x32 (ASCII '2'). Browser-safe decoder:
    // scribe.ts ships in the /browser entrypoint, where the Node-only Buffer
    // global does not exist (a Buffer reference threw and silently rejected
    // EVERY envelope a browser-side scribe saw).
    try {
        const decoded = base64.decode(s);
...
export function expiredSegments(log, policy, clock) {
    switch (policy.type) {
        case 'none':
            return [];
        case 'size': {
            // Fail-closed on a malformed bound: 'size:abc' parses to NaN and
            // NaN comparisons are always false — treat it as a config error and
            // purge NOTHING rather than guessing (data loss is irreversible).
            if (!Number.isFinite(policy.maxSegments) || policy.maxSegments < 0)
                return [];
            const excess = log.entries.length - Math.floor(policy.maxSegments);
            return excess > 0 ? log.entries.slice(0, excess) : [];
        }
        case 'count': {
            // A NaN/negative count used to expire EVERY entry (`total <= NaN` is
            // always false, so the loop never breaks) — one rooms-file typo
            // ('count:abc') silently nuked the whole scroll via kind-5.
            if (!Number.isFinite(policy.maxMessages) || policy.maxMessages < 0)
                return [];
            const maxMessages = Math.floor(policy.maxMessages);
            let total = 0;
        ...
        case 'time': {
            if (!Number.isFinite(policy.maxAgeSec) || policy.maxAgeSec < 0)
                return [];
            const cutoff = clock.nowSec() - Math.floor(policy.maxAgeSec);
