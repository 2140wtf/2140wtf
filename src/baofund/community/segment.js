...
export function decodeSegmentContent(content, ctx) {
    ...
    // P2 grace (§3.1): a message from the previous epoch may ride THIS
    // segment when the roll raced an in-flight post. Try the retained
    // previous key before salvaging/dropping.
    const prevCtx = ctx.previousEncKey && ctx.previousEpoch !== undefined
        ? { roomId: ctx.roomId, encKey: ctx.previousEncKey, routingId: ctx.routingId, epoch: ctx.previousEpoch }
        : null;
    for (let i = 0; i < manifest.messages.length; i++) {
        ...
        try {
            ...
            try {
                ...
                if (prevCtx) {
                    try {
                        envelopes.push(decodeEphemeralEvent(ev, prevCtx, 0));
                        continue; // grace window (§3.1) — accepted, no warning
                    }
                    catch { /* not the previous epoch either */ }
                }
                throw currentErr;
            }
        }
