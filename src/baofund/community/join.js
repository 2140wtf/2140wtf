...
import { buildMemberAdmissionClaim } from './memberClaim.js';
...
export async function joinRoom(conn, link, roomInfo, opts = {}) {
    ...
    const agentProof = opts.agentSecretKey && opts.agentPub
        ...
        : undefined;
    // Member-claim admission proof (F7): bind the durable member key to this
    // room + burner when the caller holds it (welcomer verifies per policy).
    const memberClaim = opts.memberSecretKey
        ? await buildMemberAdmissionClaim(opts.memberSecretKey, roomId, burnerPub)
        : undefined;
    const buildRequest = async (attempt) => ({
        ...
        ...(memberClaim ? { member: memberClaim } : {}),
