...
export interface CrossAppNip60Restore {
  ...
  /** created_at of the adopted config event (0 when no foreign config). Callers
   * should persist this as a high-water-mark: on the next restore, any config
   * with created_at <= this HWM is rejected to prevent rollback adoption.
   * When no config was adopted, HWM is 0 (no comparison needed - fresh device).
   */
 ...
 *
 * @param localConfigCreatedAt - HWM from the last adopted config.
 *   Configs with created_at <= this value are rejected to prevent rollback
 *   adoption by a hostile relay that withholds the current config.
 *   Pass 0 for a fresh device (no HWM to check against).
 */
export async function restoreCrossAppNip60Wallet(
  identitySigner: Nip60Signer,
  queryFn: (filter: NostrFilter) => Promise<NostrEvent[]>,
  localConfigCreatedAt = 0,
  ...
  try {
    ...
    // HWM guard: only adopt configs NEWER than our last known state.
    // Prevents a hostile relay from withholding the current config and
    // replaying an old (valid-signed) one that would overwrite our wallet.
    // The identical newest config is re-adopted (`<` not `<=`): re-restoring
    // the same state is not rollback, and `<=` silently rejected the
    // identity's own current config on every later refresh.
    if (newestConfig.created_at < localConfigCreatedAt) return empty;
    ...
    return { result, walletPrivkey, walletPubkey: walletSigner.pubkey, configCreatedAt: newestConfig.created_at };
  } catch (e) {
