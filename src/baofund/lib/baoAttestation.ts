/**
 * ₿AO Fund v3 - donor attestation (L2) client.
 *
 ...
 *
 * Testnet campaigns escrow on Bitcoin testnet4 / Liquid testnet; real-money
 */

import type { NostrEvent } from '@nostrify/nostrify';

import { baoApiDate } from './baoFundraising';
import { fundApiOrigin, fundFetch } from './fundHttp';
import { EVIDENCE_ENCODING, buildEvidenceTag } from './evidence/attestationEvidence';
 ...
 * the deliverable link rides as an `r` tag, notes as content. An optional
 * Bao verified-streaming commitment (`evidence` tag, see
 * lib/evidence/attestationEvidence) publishes the artifact's seekable root
 * hash so attestors can spot-check slices against it.
 */
export function buildProofOfWorkEvent(input: {
  marketId: string;
  deliverableUrl?: string;
  notes?: string;
  evidence?: { rootHashHex: string; contentLength: number };
}): EventTemplate {
  ...
  if (input.evidence) {
    tags.push(
      buildEvidenceTag({
        ...
        evidence_root_hash: input.evidence.rootHashHex,
        evidence_bytes: input.evidence.contentLength,
      }),
    );
  }
  return {
...
async function postSignedEvent<T>(signer: SignerLike, path: string, template: EventTemplate): Promise<T> {
  fundApiOrigin(); // Validate the project boundary before asking a signer.
  const event = await signer.signEvent({ ...template, created_at: Math.floor(Date.now() / 1000) });
  const json = await fundFetch<{ data: T }>(path, { method: 'POST', body: { event } });
  return json.data;
}
...
export async function submitProof(
  ...
  fundraiserId: string,
  milestoneId: string,
  input: { marketId: string; deliverableUrl?: string; notes?: string; evidence?: { rootHashHex: string; contentLength: number } },
): Promise<unknown> {
...
export async function fetchAttestation(fundraiserId: string, milestoneId: string): Promise<MilestoneAttestation> {
  const json = await fundFetch<{ data: MilestoneAttestation }>(
    `/v1/fundraisers/${encodeURIComponent(fundraiserId)}/milestones/${encodeURIComponent(milestoneId)}/attestation`,
  );
  return json.data;
}
