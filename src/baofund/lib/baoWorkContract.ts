...
export interface MilestoneEvidenceV1 {
  ...
  /** Optional Bao verified-streaming commitment (lib/evidence/attestationEvidence):
   *  the archive URL hosts the Bao ENCODING (tree + content), and
   *  evidence_root_hash is the published, seekable commitment that attestors
   *  spot-check slices against. All three fields are set together or none. */
...
export function validateMilestoneEvidenceV1(evidence: MilestoneEvidenceV1, contract: WorkContractV1): MilestoneEvidenceV1 {
  ...
  if ([evidence.evidence_encoding, evidence.evidence_root_hash, evidence.evidence_bytes].some((v) => v !== undefined)) {
    if (evidence.evidence_encoding !== "bao/blake3-v1") throw new Error('evidence_encoding must be "bao/blake3-v1".');
    requireHex(evidence.evidence_root_hash ?? "", "evidence_root_hash");
    if (!Number.isSafeInteger(evidence.evidence_bytes) || (evidence.evidence_bytes ?? 0) <= 0) throw new Error("evidence_bytes must be a positive integer.");
  }
  return evidence;
}
