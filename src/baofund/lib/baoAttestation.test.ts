...
describe('buildProofOfWorkEvent', () => {
  ...
  it('carries the Bao evidence tag only when a commitment is supplied', () => {
    const root = 'a'.repeat(64);
    const t = buildProofOfWorkEvent({ marketId: MARKET_ID, evidence: { rootHashHex: root, contentLength: 50_000 } });
    expect(t.tags).toEqual([
      ['d', MARKET_ID],
      ['evidence', root, 'bao/blake3-v1', '50000'],
    ]);
    expect(buildProofOfWorkEvent({ marketId: MARKET_ID }).tags).toEqual([['d', MARKET_ID]]);
  });

  it('refuses malformed evidence commitments (fail closed)', () => {
    expect(() =>
      buildProofOfWorkEvent({ marketId: MARKET_ID, evidence: { rootHashHex: 'XYZ', contentLength: 10 } }),
    ).toThrow(/hex/);
    expect(() =>
      buildProofOfWorkEvent({ marketId: MARKET_ID, evidence: { rootHashHex: 'a'.repeat(64), contentLength: 0 } }),
