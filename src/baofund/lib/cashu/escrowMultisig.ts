...
import { decodeCashuToken, normalizeMintUrl, type DecodedTokenEntry } from './tokenUtils';
...
export function buildMultisigEscrowLock(req: MultisigEscrowLockRequest): MultisigP2pkOptions {
  ...
  if (!Number.isSafeInteger(req.locktime) || req.locktime <= Math.floor(Date.now() / 1000)) {
    // A past or current locktime yields an INSTANTLY-refundable deposit -
    // every depositor could reclaim their stake before any outcome window
    // even opens. The docstring always promised a "safe future timestamp";
    // now the code enforces it.
 ...
 * tolerated here (validation decides what is acceptable); duplicate
 * security-critical tags are rejected because different NUT-11 consumers
 * (first-occurrence vs last-occurrence) can read different spending
 * conditions from the same secret.
 */
export function parseMultisigLockSecret(secret: unknown): ParsedMultisigLock | null {
  ...
  try {
    ...
    const seenSecurityTags = new Set<string>();
    for (const tag of rawTags) {
      if (!Array.isArray(tag) || tag.length === 0 || typeof tag[0] !== 'string') return null;
      const [, ...values] = tag as [string, ...unknown[]];
      if (['pubkeys', 'refund', 'locktime', 'n_sigs', 'n_sigs_refund'].includes(tag[0])) {
        if (seenSecurityTags.has(tag[0])) return null;
        seenSecurityTags.add(tag[0]);
      }
      switch (tag[0]) {
        case 'pubkeys':
...
export function validateMultisigEscrowDeposit(
  ...
  // A duplicated proof (same secret) counts twice in `entry.amount` but is
  // only spendable ONCE at the mint: the escrow would credit 2x while holding
  // 1x. Reject repeats before any accounting.
  const seenSecrets = new Set<string>();
  for (const entry of entries) {
    for (const proof of entry.proofs) {
      const secretKey = String((proof as { secret?: unknown }).secret ?? '');
      if (!secretKey || seenSecrets.has(secretKey)) {
        return fail('Deposit token contains duplicate proofs', amount);
      }
      ...
      if (lock.requiredRefundSignatures !== 1) {
        // NUT-11 counts each pubkey at most once: a single depositor key can
        // never satisfy 2+ required refund signatures, so such a lock is
        // permanently unrefundable (and the operator is not an allowed
        // signer on the refund path).
        return fail('Deposit refund requires more than one signature - the depositor cannot self-refund', amount);
      }
