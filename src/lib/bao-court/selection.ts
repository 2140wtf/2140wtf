/**
 * Deterministic, verifiable jury selection for the BAO Court / Juror Mode.
 */
...
export function filterEligibleJurors(
  ...
  return pool.filter((j) => {
    ...
    // Only jurors with a confirmed stake commitment are eligible.
...
export function selectJuryWithBackups(
  ...
  const seed = deriveSelectionSeed(p.disputeEventId, p.blockHash);
  const scored = scoreAndSort(eligible, seed);

  // Everyone with a confirmed stake who is not selected stays as a backup.
  return {
