 ...
 * multipliers / offsets that the decay, item-effect, XP, and reward systems
 * apply at read time.
 */
...
export interface CategoryAbilityBonuses {
  ...
  /** Multiplier applied to XP gains from care/daily missions. */
  xpMultiplier: number;
 ...
 * Used by the Ditto Blobbi nature-sync ability.
 */
 ...
 *
 * The returned object is ready to be consumed by decay, item-effect, XP, and
 * reward calculations. Callers should apply each multiplier/offset at the point
 * where the corresponding value is computed.
 *
 * For Ditto Blobbi, the daytime XP bonus is included when `isLocalDaylight()`
...
export function getCategoryAbilityBonuses(
  ...
  switch (category) {
    case 'ditto-blobbi': {
      const daylightXpMultiplier = isLocalDaylight(now) ? 1.1 : 1;
      return {
        ...BASE_BONUSES,
        happinessDecayMultiplier: 0.85,
        xpMultiplier: daylightXpMultiplier,
        statCapBonus: 5,
      };
    }
