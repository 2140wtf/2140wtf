...
export function useJurorSession(
  ...
  useEffect(() => {
    ...
    async function tryComputeKey() {
      if (completed) return;
      // Decrypt BEFORE any readiness check: canComputeKey() requires decrypted
      // shares, so gating decryption on it deadlocks the first valid share set.
      await session.decryptShares();
      session.verifyShares(commitmentEventIdsRef.current);
      session.resolveComplaints();
      const dkgPhaseNow = session.getPhase();
      if (dkgPhaseNow === 'failed' || !session.canComputeKey()) {
        setDkgPhase(dkgPhaseNow);
        return;
      }
  ...
  // Real-mode vote subscription: track each juror's earliest commit and tally
  // only reveals that match that juror's exact prior commit. A reveal without
  // a matching commit is ignored, never counted.
  useEffect(() => {
    ...
    const filter: NostrFilter = {
      kinds: [BAO_COURT_VOTE_COMMIT_KIND, BAO_COURT_VOTE_REVEAL_KIND],
      authors,
      '#dispute': [dispute.disputeId],
      since,
    };

    const revealEvents: NostrEvent[] = [];
    // First commit per roster index wins; conflicting later commits are ignored.
    const commitsByIdx = new Map<number, { commitHash: string; createdAt: number }>();
    if (myVoteCommit) {
      commitsByIdx.set(myJurorIdx, { commitHash: myVoteCommit, createdAt: 0 });
    }

    function rosterIdxFor(pubkey: string): number | null {
      const juror = selectedJurors.find((j) => j.nostrPubkey === pubkey);
      return juror ? juror.idx : null;
    }

    function computeTally() {
      const votes: JurorVote[] = [];
      const seen = new Set<number>();
      const addReveal = (reveal: {
        idx: number;
        pubkey: string;
        outcome: string;
        salt: string;
        ...
        const commit = commitsByIdx.get(reveal.idx);
        if (!commit) return; // reveal without a known commit
        if (commit.createdAt > reveal.createdAt) return; // commit must predate the reveal
        if (hashCommit(reveal.outcome, reveal.salt) !== commit.commitHash) return; // mismatch
        seen.add(reveal.idx);
        votes.push({
          idx: reveal.idx,
      ...
      for (const event of revealEvents) {
        ...
        const rosterIdx = rosterIdxFor(event.pubkey);
        if (rosterIdx === null || rosterIdx !== parsed.jurorIdx) continue;
        addReveal({
          idx: parsed.jurorIdx,
    ...
    async function handleEvent(event: NostrEvent) {
      if (event.kind === BAO_COURT_VOTE_COMMIT_KIND) {
        const parsed = parseVoteCommitEvent(event);
        if (!parsed || parsed.disputeId !== dispute.disputeId) return;
        if (event.pubkey === myPubkey) return; // local commit state is authoritative
        const rosterIdx = rosterIdxFor(event.pubkey);
        if (rosterIdx === null || rosterIdx !== parsed.jurorIdx) return;
        if (commitsByIdx.has(parsed.jurorIdx)) return; // first commit wins
        commitsByIdx.set(parsed.jurorIdx, {
      ...
      if (event.kind === BAO_COURT_VOTE_REVEAL_KIND) {
  ...
  }, [realMode, phase, user, myVoteReveal, myVoteCommit, peerPubkeys, selectedJurors, dispute.disputeId, myJurorIdx]);
  ...
  const publishFrostCommitment = useCallback(async () => {
    ...
    try {
      ...
      if (!realMode) {
        throw new Error(
          "Cross-juror FROST commitment is only available in demo or real ceremony mode.",
        );
      }
      if (!dkgRecordRef.current) return;
      if (independentSigningRef.current) {
        // Exactly one FROST nonce commitment per signing attempt; never republish.
        return;
      }
      if (!tally) {
        throw new Error(
          "Real signing requires the frozen vote tally first; refusing to fall back to a local outcome.",
        );
      }
      ...
      // The session creates the single nonce commitment for this attempt and
      // keeps its secret nonces for the reveal; only this event is published.
  ...
  const publishFrostReveal = useCallback(async () => {
    if (!user || !myShareRef.current || !dkgRecordRef.current) return;
    // frostCommitmentRef only exists in demo mode; real mode stores its single
    // nonce commitment inside the IndependentSigningSession.
    if (demoMode && !frostCommitmentRef.current) return;
    setIsPending(true);
    try {
  ...
  const aggregateAndPublishAttestation = useCallback(async () => {
    ...
    try {
      ...
      if (!tally) {
        throw new Error("Cannot aggregate without a frozen vote tally.");
      }
      const session = independentSigningRef.current;
      if (session.outcome !== tally.outcome) {
        // The verdict changed after nonce commitments were created. Signing a
        // different message requires a brand-new signing attempt with fresh
        // nonces; never reuse this attempt's commitments for another outcome.
        throw new Error(
          "The tallied verdict changed after FROST commitments were published; " +
            "this signing attempt must abort and restart as a new attempt.",
        );
      }
