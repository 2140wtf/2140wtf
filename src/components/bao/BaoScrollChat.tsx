...
export function BaoScrollChat({ lockedRoom, embedded }: BaoScrollChatProps) {
  ...
  useEffect(() => {
    ...
    // ANTI-LEAK GUARD: in locked-room (trollbox) mode the relay is pinned.
    // Fail closed before any socket exists if the directory ever drifts.
    if (lockedRoom) assertTrollboxRelayPinned(lockedRoom);
  ...
  const addRoomFromLink = useCallback(() => {
    const link = normalizeJoinInput(joinLinkInput.trim());
    if (!link) return;
    try {
