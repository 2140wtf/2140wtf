...
const PHASES: { id: AppealPhase; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
  { id: "selection", label: "Jury picked", icon: Users },
  { id: "dkg", label: "Setup", icon: KeyRound },
  { id: "vote-commit", label: "Vote", icon: Vote },
  { id: "vote-reveal", label: "Count", icon: ScrollText },
  { id: "signing", label: "Sign", icon: FileSignature },
  { id: "attestation_published", label: "Verdict", icon: CheckCircle2 },
];
...
/** Plain-language explanation shown in the guided walkthrough. */
function phaseDescription(phase: AppealPhase): string {
  switch (phase) {
    case 'selection':
      return 'Your jury is picked and every juror\'s refundable deposit is locked. Next you\'ll set up the jury\'s shared key — one click, and no single juror ever holds it.';
    case 'dkg':
      return 'Setup complete. The jury now shares one public key, and no single device knows the secret behind it.';
    case 'vote-commit':
      return 'Review the evidence and lock in your vote. Votes stay sealed until everyone has voted, so nobody can change theirs after seeing the others.';
    case 'vote-reveal':
      return 'Votes are unsealed and counted — the majority outcome wins. In this practice round all jurors vote the same way, so the result is unanimous.';
    case 'signing':
      return 'The jury combines its signatures into one final verdict that anyone can verify.';
    case 'attestation_published':
      return 'Done — the verdict is published, signed by the whole jury.';
    default:
      return '';
  }
}

/** Original technical explanation, kept behind the "Technical details" toggle. */
function phaseTechnicalDetails(phase: AppealPhase): string {
  switch (phase) {
    case 'selection':
    ...
    case 'signing':
      return 'Jurors combine their partial signatures to produce one valid attestation under the group public key (threshold FROST).';
    case 'attestation_published':
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-start justify-between gap-3 pr-6">
            <DialogTitle className="leading-snug">Jury session</DialogTitle>
            {demoMode ? (
        ...
        <div className="space-y-6">
          ...
          {demoMode && demoPace === 'guided' && (
            <div className="rounded-md bg-muted p-4 text-sm text-muted-foreground space-y-2">
              <p>{phaseDescription(state.phase)}</p>
              <Collapsible open={detailsOpen} onOpenChange={setDetailsOpen}>
                <CollapsibleTrigger className="flex items-center gap-1 text-xs font-medium text-foreground/70 hover:text-foreground transition-colors">
                  Technical details
                  {detailsOpen ? <ChevronUp className="size-3" /> : <ChevronDown className="size-3" />}
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <p className="pt-1 text-xs">{phaseTechnicalDetails(state.phase)}</p>
                </CollapsibleContent>
              </Collapsible>
            </div>
          )}
            ...
            <div className="space-y-4">
              <Card>
                <CardContent className="p-4 space-y-3">
                  ...
                  <div>
                    <span className="text-sm text-muted-foreground">Challenged decision</span>
              ...
              <div className="space-y-2">
                <h3 className="text-sm font-semibold">Your jury</h3>
          ...
          {state.phase === "dkg" && (
            <div className="space-y-4 text-center">
              ...
              <h3 className="text-lg font-semibold">
                {sessionProgress.dkgPhase === 'complete' ? 'Your jury is ready' : 'Setting up your jury…'}
              </h3>
              <p className="text-sm text-muted-foreground">
                {sessionProgress.dkgPhase === 'complete'
                  ? 'The jury\'s shared key is set up — no single device holds the secret.'
                  : 'The jury is creating its shared key. This usually takes a moment.'}
              </p>
              <Collapsible>
                <CollapsibleTrigger className="inline-flex items-center gap-1 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors">
                  Technical details
                ...
                <CollapsibleContent className="space-y-3 pt-2">
                  ...
                  {state.groupPubkeyXOnly && (
                    <div className="font-mono text-xs break-all rounded-md bg-muted p-3 text-left">
                      {state.groupPubkeyXOnly}
                    </div>
                  )}
                </CollapsibleContent>
          ...
          {state.phase === "vote-commit" && (
            <div className="space-y-4">
              <h3 className="text-sm font-semibold">Voting open — review the evidence and pick the correct outcome</h3>
              ...
              <p className="text-xs text-muted-foreground text-center">
                Your vote is sealed once locked — you can't change it afterwards.
              </p>
            </div>
          )}
          ...
          {state.phase === "signing" && (
            <div className="space-y-4 text-center">
              ...
              <h3 className="text-lg font-semibold">Sign the verdict</h3>
              <p className="text-sm text-muted-foreground">
                Add your signature so the jury can publish the final verdict.
              </p>
              {realMode && (
                <Collapsible>
                  <CollapsibleTrigger className="inline-flex items-center gap-1 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors">
          ...
          {state.phase === "attestation_published" && (
            <div className="space-y-4 text-center">
              ...
              <h3 className="text-lg font-semibold">Verdict published</h3>
              {state.attestation && (
