    ...
    return (
      <div className={cn('grid grid-cols-1 sm:grid-cols-2 gap-6 py-4', className)}>
        {Array.from({ length: 4 }).map((_, i) => (
          <Skeleton key={i} className="h-[420px] w-full rounded-xl flex items-center justify-center" />
        ))}
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {filteredPolls.map((poll) => (
          <div key={poll.id} className="flex flex-col gap-3">
            <HostedPollCube pollId={poll.id} title={extractTitle(poll)} className="h-[420px] flex items-center justify-center" />
