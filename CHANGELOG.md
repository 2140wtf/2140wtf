...
- Cashu seed decryption errors now explain recovery options and warn before destructive regeneration.
- Feed events persist in IndexedDB for instant warm and offline loads without false empty states.
...
- Durable multi-mint Lightning payments with crash recovery.
- Animated BC-UR Cashu QR transfers across supported wallets.
- Project-first ₿AO Fund browsing, public milestones, and repository links.
- Nostr-native project workspaces and agent collaboration tools.
- Headless ₿AO CLI and MCP support for automated agents.
- Community fundraising, compute-credit milestones, and donor attestations.
- Relay-first market creation, discovery, trading, and position views.
- Expanded wallet, Pets, battle, notification, and installation features.
...
- Updated campaign runner fees to 2.14%, 4.21%, and 10%.
- Clarified beta, custody, escrow, and payment-risk messaging.
- Unified ₿AO Community and funding terminology.

### Fixed
- Reusable BOLT12 issuance, settlement, replay safety, and recovery.
- Concord authorization, rekeying, membership, and account isolation.
- NIP-01 replaceable-event deduplication and strict BIP32 validation.
- Native-safe downloads, links, signer persistence, and API authentication.
- Zap confirmation, Cashu deposit recovery, and payment error reporting.
- Compute-credit claim, redemption, privacy, and account-switch handling.
- Pet hatching, naming, battle settlement, and mobile interaction failures.
- Market history, pagination, odds, and balance accuracy.
- Invite routing, relay hints, dialog scrolling, and notification state.
