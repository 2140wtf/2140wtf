...
### Added
- Cashu mint discovery — the wallet now searches Nostr recommendations instead of limiting users to three hardcoded mints, with local/global discovery, health and protocol-capability checks, community ratings/reviews, mint detail pages, and one-click add/remove actions. Mint operators remain custodians, so the UI includes explicit trust and research-risk warnings.
- Modern Cashu payment interoperability — wallet deposits and payments support BOLT12 offers alongside BOLT11, NUT-17 live settlement subscriptions cover both mint and melt quotes with polling fallback, NUT-19 safely replays a byte-identical failed request once only for exact endpoints whose mint advertises cached responses, NUT-20 protects BOLT11 mint quotes with a fresh per-quote key where supported, and NUT-27 synchronizes the user's mint list through Nostr.
- NUT-15 multi-mint Lightning payments — invoices can be split across two to four compatible mints with exact millisatoshi allocation, an explicit per-mint amount and maximum-fee confirmation, simultaneous dispatch, encrypted crash journals, quarantined inputs, and reload-safe reconciliation for every participating mint.
- Animated Cashu QR transfers (NUT-16) — larger ecash tokens are emitted and scanned as interoperable BC-UR animated frames in the main wallet, ₿AO wallet, and Pets wallet; small tokens remain ordinary static QR codes and reduced-motion users get manual frame controls.
- Public, project-first ₿AO Fund browsing — opening Fund now starts with inspectable projects and milestones, then offers clear donation and campaign-creation paths. Embedded repository metadata is rendered as a safe public review link, while the page explains that private team discussion happens separately inside a ₿AO community and never hides public progress behind the compute-credit check.
- Research-beta notice shared across money surfaces — 2140.wtf and ₿AO Fund are clearly identified as active beta research projects with an instruction to use the technology at your own risk and avoid large amounts.
...
### Changed
- ₿AO Fund now explains the complete project → public funding → milestone → private teamwork flow with responsive step cards and expandable Why/How/Where guidance. Every open project exposes its donor action without requiring expansion or the client-side agent check; campaign creation opens behind a clearly scoped proof-of-work screen, while the UI explicitly distinguishes that bypassable local anti-spam check from bao.markets authorization. Campaign repository metadata becomes a safe review link, and logged-out compute-credit cards explain how donors can participate.
...
### Fixed
- BOLT12 offers no longer strand later payments after their first issuance: the wallet claims each newly paid increment, records and syncs the amount actually issued, keeps reusable offers available, and restores interrupted deterministic output windows. Each offer now uses a fresh encrypted quote key instead of the long-lived NIP-60 wallet identity, preventing a mint from trivially linking the wallet's deposits.
- Concord community transport now rejects mismatched stream public/private keys, malformed conversation keys, authorless or out-of-capability queries, and malformed AUTH acknowledgements. Account changes withhold community consumers until old sessions and authorization caches are revoked, closing a cross-account effect-order race.
- Native file and link handling — saving a generated NOSTR PET photo now writes the PNG bytes to the device Documents directory instead of sharing an unreadable WKWebView `blob:` URL; BTC Map merchant websites and social links use the native-safe URL bridge; middle-clicked post windows use `noopener,noreferrer` on web.
- Nostr replaceable/addressable event deduplication now follows NIP-01 for exact timestamp ties by selecting the lexicographically lowest event id, independent of relay arrival order.
- BIP32 path parsing now rejects partial numbers, empty/negative/decimal components, unsafe integers, and child indices outside the 31-bit range while accepting all standard hardened markers (`'`, `h`, `H`).
- ₿AO signet Cashu deposits now use the canonical bao.markets proxy mint URL throughout receive, restore, and pending-quote recovery. Pending BOLT11 deposits subscribe for settlement, remain manually confirmable, and are guarded against duplicate issuance; the live proxy advertises the required BOLT11, NUT-17, and NUT-20 capabilities.
...
- Pet naming after hatching is no longer a dead end — the name field could not be typed into on iOS/Android (the full-screen ceremony overlay is `user-select: none`, which makes WebKit treat inputs inside it as read-only), tapping while the naming question typed itself out did nothing, and if anything went wrong at the naming step there was no way out of the ceremony at all. The field now opts back into text selection (`select-text` + explicit `-webkit-user-select: text`), sits in a real form so Enter / the keyboard's Done key submits the name and returns you to the app, the naming question can be tapped to skip the typewriter and reveal the field immediately, and a Back button is available in every interactive ceremony phase (cracking, dialog, naming) so you can always leave
...
- Sealed group chat flood/bounds hardening: future-epoch group events are only buffered within a 100-epoch gap and 30-day age, the per-group pending buffer is capped at 200 with oldest-evicted, and sendMessage now captures epoch + exporter secret atomically under the group lock so a concurrent key rotation can't tag an event with the new epoch while encrypting it with the old epoch's key

- Deploy workflow now passes `VITE_PETS_BATTLE_ESCROW_PUBKEY` and `VITE_PETS_BATTLE_ESCROW_URL` into the build env (previously neither was wired, so the built app never learned the escrow operator's pubkey or where to POST /release; values live in repo Actions variables — the URL must be HTTPS since Pages serves the app over HTTPS and browsers block http:// API calls as mixed content). Mirrors bao_fund 912bb73
- Invite links are built on the canonical public origin — link creation now uses the configured share origin (https://2140.wtf) instead of the page's own origin, so a link minted on a dev machine (localhost) or inside the Android APK (the WebView's unreachable local origin) no longer resolves nowhere for its recipient. Every client mints https://2140.wtf/invite/… links, and the native-build fallback origin was corrected from an upstream-fork leftover
- Copied post links resolve for everyone — "Copy Link to Post" (and the event ID in the JSON view) now embeds up to 3 NIP-19 relay hints (the relays you actually read the event from) in the nevent/naddr, so a recipient whose relays never saw the post — and whose author's outbox relays don't hold it either — opens the post instead of "Event not found". The router and detail pages already honored hints; only the encode side was missing them
- New-₿AO dialog is fully scrollable — the whole module now scrolls with generous bottom padding, so the relay list's tail and the Create button are always reachable without fullscreening the browser (previously the bottom was cut off on shorter windows)
...
### Changed
- Merchants market puts listings with images first — the /market grid now leads with listings that have photos under every sort order (newest, price, merchant), so bare text listings no longer bury the visual ones
- ₿AO Fund Campaigns-tab demo banner de-yellowed (ported from bao_fund): the loud amber dashed box is now a quiet neutral notice (thin border + muted background, plain heading) — same safety copy (demo rails, recorded-not-settled, how to claim demo sats), no more yellow block shouting over the page
- Nobody joins a ₿AO nameless — the invite page now requires a display name before the join fires: the agent create-my-key fast path makes the agent name a required field (published as the key's kind-0 profile, `bot: true` now optional separately), and a logged-in member without a published name sets one as part of accepting the invite (saved as their public profile name — agents that log in with an existing nameless key get the same gate). Nameless keys already inside render in chat as `anon-<npub8>` instead of "Anonymous", so a busy room stays readable (who said what, who is who); AGENTS.md documents the name rule for headless agents
- Invite links default to single use — the safe option is now the preselected one (a single-use link dies after the first join), and the link options (expiry + label) are always visible instead of hidden behind a "Link options" expander, so the tradeoffs are seen rather than tucked away. Live links are numbered by creation order — #1 is the oldest live link on top, the newest appends at the bottom, and a revoke shifts the rest up so the sequence stays clean (same numbering in the community's Invites view)
- Creating a ₿AO now requires an explicit relay choice — no relays are pre-ticked, and the Create button stays disabled until you tick at least one (or add your own). The pick doubles as confirming the reach-vs-privacy tradeoff explained right above the list, reworded to be plain rather than scary: messages are sealed either way, the everyday app relays are a fine home for most communities, and only super privacy-focused groups benefit from trimming to a single relay they control (ideally with NIP-42 read auth). A "Select all" shortcut keeps the old one-tap max-reach setup. Previously the full feed relay set was silently pre-selected behind an "advanced" chevron
- Battle escrow explainer rewritten in plain, jargon-free language (ported from bao_fund): states up front that 2140 holds no access to the funds and why 2140 runs the escrow at all (a neutral referee so two strangers can play for sats without trusting each other), that both players' apps push the result and the pot settles automatically when the reports agree, and that a crashed game or dropped connection returns both contributions after 24 hours via the automatic refund rule baked into the lock — with the 24-hour window existing so either side can contest a wrong result before money moves. Honest limits (operator collusion, mint custody) kept in plain words
...
### Added
- CLI kind-0-on-join (ported from bao_fund): the headless driver publishes a kind-0 profile ({"name": <identity>, "bot": true}) as part of BOTH create and join — agent identities announce their name on entry, matching the web join path (nameless keys render as anon-<npub8>). Shared publishAgentProfile in scripts/chat-core.ts; verified live against the relay
- Agent chat-core + MCP server (ported from bao_fund): the headless driver gains idempotent send (`say --key K` — d-tag + own-history pre-scan, a retry dedupes), the mention interrupt (`wait [--timeout S] [--all]` — first NEW message mentioning me; timeout is a sentinel with exit code 2), and task orchestration (`orch claim|progress|done|blocked|ack|handoff|show`) with THE shared claim tie-break in `src/concord-v2/lib/orchestration.ts` (first CLAIM wins, ties by lowest rumor id, 30-min PROGRESS staleness, DONE/BLOCKED only by the claimant). Mention detection is client-side post-decrypt with npub1 tokens auto-promoted to p-tags. New `public/bao-chat-mcp.mjs` — zero-repo stdio MCP server (8 tools incl. wait_for_message + orch) built on the SAME `scripts/chat-core.ts` as the CLI, with a per-identity JSONL audit log; `public/CHAT_PROTOCOL.md` documents the wire with real sniffed event shapes and the sealed-₿AO vs bao.markets-plaintext fork up front
- Invite links show usage status — each of your live links now reports "Used ×N · first <date>" or "Not used yet" (both the invite dialog and the community Invites view), matched client-side from the sealed Guestbook's join commitments. This is not a privacy leak: the Guestbook is sealed so relays only ever see ciphertext (the scan runs post-decrypt on your own device), and the sha256 token commitment a join cites is matchable only by whoever holds the link's 128-bit token — the link's creator. The only thing it tells the inviter is join timing, no more than watching the member list
- AI milestone verifier UI on ₿AO Fund (shipped by the kimi-verifier session): campaign creation gets an AI-verifier model picker with up-front fee disclosure, milestone cards get a verification score button and a donation progress bar, and campaigns carry subcategory tags — the frontend half of the Routstr-scored milestone verification (score events, fee deduction at release) that shipped on the bao.markets side
- Zero-repo agent join (ported from bao_fund) — `public/bao-agent.mjs` is a fully self-contained build of the headless ₿AO driver (every dependency inlined, runs on bare Node 22+): an agent with nothing but curl + node joins with `curl -sSL <raw-url> -o bao-agent.mjs && node bao-agent.mjs join "<invite-url>" --as <name>` — no clone, no npm install, no browser. Rebuild after touching the driver or concord-v2 with `npm run agent:bundle`; AGENTS.md leads with the zero-repo path
- One-command agent join (ported from bao_fund) — new `npm run agent -- join "<invite-url>" --as <name>` script builds and runs the headless ₿AO CLI in a single step (key created on first use, stored in ~/.concord-live), and AGENTS.md opens with a copy-paste "Join in 60 seconds" block that steers agents away from running the TS driver directly (path aliases only resolve through the rolldown build)
- Agent-audience invite links + the agent fast path (ported from bao_fund): the invite dialog now asks who a link is for — human or AI agent — and agent links land on a machine-first join page: a scrapeable join card (bundle coordinate, bootstrap relays, /AGENTS.md) for browserless agents, paste-your-nsec / extension / bunker quick login that joins the moment it lands, and a one-click create-my-key flow (keypair generated, `bot: true` profile published, the nsec shown exactly once with the join held until it's stored). Agent-gated ₿AOs are handled in-page — the join grinds the proof-of-work itself. The audience rides inside the encrypted invite bundle and survives rekey refreshes; AGENTS.md documents the fast path
- 2-of-3 multisig battle escrow — the non-custodial ₿AO escrow primitive (ported from bao_fund), with real-sats pet battles as its first consumer. Stakes lock to THREE keys (both players + the escrow operator) and need ANY TWO signatures to move: the operator can never take the funds (it only co-signs the winner's claim), the outcome is attested by both players (mutual attestation, see Changed), and an abandoned battle self-heals — after the 24h locktime each player reclaims their exact stake with their own key, no operator needed (one-click "Reclaim my stake" once unlocked, plus an automatic stale-deposit refund sweep on the battle page). Battle setup shows a "How ₿AO escrow protects your sats" explainer with the honest trust limits.
...
- Market cards show payment-rail chips (⚡/SPARK/CASHU/LIQUID/₿/FEDIMINT) parsed from the API's payment_rails, matching the bao.markets card footer
- My trades covers SMJ too — parimutuel bets show in the positions list with an SMJ pool badge (new GET /v1/smj/positions backend route), and SMJ markets get a REAL odds-over-time chart built from the public bet feed (new GET /v1/smj/:id/history backend route) instead of the 'No trade history yet' gap
...
- Install-app card in Settings (plus the existing pills on the landing hero, Help/FAQ, and mobile drawer)
- Kind-38000 market definitions documented in NIP.md for other clients (display-only; escrow/settlement out of scope)
...
- Remote-signer guidance — a one-time "choose Always allow" tip at login and a "Why does my signer keep asking for approvals?" FAQ
- Pet battles go full manga — 22 moves from button combos: slash chains (×3 finisher), uppercut (jump+sword), sweep (block+sword), dash-slash and spin-slash (sword mid-dash/backdash), the massive hammer (sword+fireball together — a huge overhead mallet slam), air slash, air swirl (toward+sword in air), salto (away+sword in air), dive kick (block+sword in air), flip-over (jump toward a close opponent — somersault over their head and land behind them), massive fireball (block+fireball), anti-air lob (jump+fireball), air fireball, dash fireball, plus double-tap dashes, super jump (block+jump), and block. Every fighter now carries a big always-visible sword with ghosted swing trails and slash crescents, and hits land with manga effects — speed lines, impact starbursts, screen shake, ground shadows, and SFX text pops ("MEGA FIRE!!", "HAMMER!!"). Base movement is slower and weightier so dashes and specials are the burst option; the AI uses the air game and massive fireballs too, the whole move set syncs over remote battles, and the controls help lists the combos
...
- Routstr redeem error no longer claims "the token was not spent" when the mint couldn't be reached to check — the unknown case now says so and tells you the credit may have landed server-side (ported from bao_fund)
- Compute-credit open requests can no longer be pinned to the top of the list by a future-dated event (created_at is clamped to now for sorting), and the query limit is raised to 1000 so a junk-request flood can't push legitimate requests out of the relay window (ported from bao_fund)
- Campaign list fetch follows the API pagination envelope — only page 1 was read before, hiding campaigns once the list outgrew one page (ported from bao_fund)
